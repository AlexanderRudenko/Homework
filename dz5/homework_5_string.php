...
echo '<br>';
if (strrev($str) == $str) {
    echo 'Пара слов: ', $str, ' являетя палиндромом';
} else {
    echo 'Пара слов: ', $str, ' не является палиндромом';
}


/*
32. Дан массив $arr. Найдите в нем все пары слов-палиндромов (одинаково читаются во всех направлениях, кот-ток, нос-сон).
...
*/
echo '<br><br> ----- 32. Дан массив arr. Найдите в нем все пары слов-палиндромов<br>';
$arr = array('slon', 'nos', 'tok', 'tak', 'kot', 'pir', 'mir', 'son', 'rim');
$arr_len = count($arr) - 1;
foreach ($arr as $key => $value) {
    ...
    for ($i=$key+1;$i<=$arr_len;$i++){
    if($value == strrev($arr[$i])){
        echo $value,' - ',$arr[$i],'<br>';
    }
    }
}
...
Обратите внимание на то, что при обратном чтении игнорируются пробелы, запятые, дефисы, тире и большие буквы (подсказка: значит сначала нужно привести строку к стандартному виду — удалить лишние символы, привести все к нижнему регистру).
*/
echo '<br><br> ----- 33. Определите является ли фраза палиндромом. Примеры: \'A roza upala na lapu Asora\'. <br>';
$str = 'A roza upala na lapu Asora';
$str_n = strtolower($str_n);
$str_n = trim($str_n,' ');
if ($str_n ==strrev($str_n)){
    echo 'Строка ', $str, ' является фразой палндромом';
}
else{
    echo 'Строка ', $str, ' не является фразой палндромом';
}
...
xxxxx
*/
echo '<br><br> ----- 34. Нарисуйте пирамиду, как показано на рисунке. Решите задачу с помощью одного цикла и функции str_repeat. <br>';
...
55555
 */
echo '<br><br> ----- 35. Нарисуйте пирамиду из цифр, как показано на рисунке. Решите задачу с помощью одного цикла и функции str_repeat. <br>';
for($i=1;$i<=9;$i++){
